...
public class AppDbContext: DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<User>(builder =>
        {
            ...
            builder.Property(u => u.Role).IsRequired().HasConversion<string>().HasMaxLength(50).HasDefaultValue(UserRole.Reader);
            builder.Property(u => u.Status).IsRequired().HasConversion<string>().HasMaxLength(50).HasDefaultValue(UserStatus.Active);
        ...
        modelBuilder.Entity<Chapter>(builder =>
        {
            ...
            builder.Property(c => c.Status).IsRequired().HasConversion<string>().HasMaxLength(50).HasDefaultValue(ChapterStatus.Created);
            builder.HasIndex(c => new { c.SeriesId, c.ChapterNumber }).IsUnique();
            
            builder.HasMany(c => c.MangaTasks).WithOne(t => t.Chapter).HasForeignKey(t => t.ChapterId).OnDelete(DeleteBehavior.Cascade);
            builder.HasMany(c => c.Feedbacks).WithOne(f => f.Chapter).HasForeignKey(f => f.ChapterId).OnDelete(DeleteBehavior.Restrict);
            builder.HasMany(c => c.ChapterVotes).WithOne(v => v.Chapter).HasForeignKey(v => v.ChapterId).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        modelBuilder.Entity<MangaTask>(builder =>
        {
            ...
            builder.Property(t => t.Status).IsRequired().HasConversion<string>().HasMaxLength(50).HasDefaultValue(MangaTaskStatus.Processing);
            
            builder.HasOne(t => t.Income).WithOne(i => i.MangaTask).HasForeignKey<Income>(i => i.MangaTaskId).OnDelete(DeleteBehavior.Cascade);
            builder.HasMany(t => t.Feedbacks).WithOne(f => f.MangaTask).HasForeignKey(f => f.MangaTaskId).OnDelete(DeleteBehavior.Restrict);
        });
        
        modelBuilder.Entity<Income>(builder =>
        {
            builder.Property(i => i.Amount).IsRequired().HasColumnType("decimal(18,2)");
            builder.Property(i => i.Status).IsRequired().HasConversion<string>().HasMaxLength(50).HasDefaultValue(IncomeStatus.Pending);
        ...
        modelBuilder.Entity<ChapterVoting>(builder =>
        {
            ...
            builder.Property(v => v.Status).IsRequired().HasConversion<string>().HasMaxLength(50).HasDefaultValue(VoteStatus.Active);

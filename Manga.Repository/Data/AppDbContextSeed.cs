    ...
    private static async Task SeedChaptersAsync(AppDbContext dbContext)
    {
        ...
        var addChapter = (string seriesTitle,int chapterNumber ,string manuscriptFileUrl, string chapterFileUrl, string title, string summary, int page, ChapterStatus status) => {
            ...
            if(s != null && s.PublishingSchedule != null) {
                ...
                DateTimeOffset deadline = string.Equals(period, "weekly", StringComparison.OrdinalIgnoreCase) ? publishDateOfChap.AddDays(-2) : publishDateOfChap.AddMonths(-1);
                DateTimeOffset createdAt = string.Equals(period, "weekly", StringComparison.OrdinalIgnoreCase) ? publishDateOfChap.AddDays(-7) : publishDateOfChap.AddMonths(-1);
        ...
        foreach (var chapter in chapters)
        {
            ...
            for (int i = 0; i < taskCount; i++)
            {
                ...
                tasks.Add(new MangaTask
                {
                    ...
                    AssignedAt = chapter.CreatedAt
                });
                
                incomes.Add(new Income { Id = Guid.NewGuid(), MangaTaskId = taskId, Amount = 100.00m + random.Next(100), Status = IncomeStatus.Pending, CreatedAt = chapter.CreatedAt });
            }
        }
